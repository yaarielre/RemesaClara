...
const categories = ["Todas", "Comparativas", "Por País", "Educación"];
...
export default function Comparativas() {
  ...
  const filtered = data.comparativas.filter((c) => {
    const matchesCategory = selectedCategory === "Todas" || c.categoria === selectedCategory;
    const matchesSearch = c.titulo.toLowerCase().includes(searchTerm.toLowerCase()) || c.subtitulo.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="bg-surface-dark min-h-screen">
      <section className="bg-hero py-16 md:py-20 border-b border-border/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-4">{t("comparativas_page.titulo")}</h1>
            <p className="text-lg text-white/70">{t("comparativas_page.subtitulo")}</p>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((comp) => <ComparisonCard key={comp.id} comparativa={comp} />)}
        </div>

        {filtered.length === 0 && (
          <div className="text-center py-16">
            <p className="text-lg text-text-light">{t("comparativas_page.no_resultados")} "{searchTerm}"</p>
          </div>
        )}
